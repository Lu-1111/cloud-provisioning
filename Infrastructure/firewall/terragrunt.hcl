...
dependencies {
  paths = ["${get_parent_terragrunt_dir()}/vpc"]
}
...
terraform {
  ...
  extra_arguments "common_var" {
    ...
    arguments = ["-var-file=${get_terragrunt_dir()}/vars/${local.common.remote_state.config.project}-${local.common.remote_state.config.location}/firewall.tfvars"]
    required_var_files = [

...
dependencies {
  paths = ["${get_parent_terragrunt_dir()}/vpc/native"]
}
...
terraform {
  ...
  extra_arguments "common_var" {
    ...
    arguments = ["-var-file=${get_terragrunt_dir()}/vars/${local.common.remote_state.config.project}-${local.common.remote_state.config.location}/subnets.tfvars"]
    required_var_files = [
